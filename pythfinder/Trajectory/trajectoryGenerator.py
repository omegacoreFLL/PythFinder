    ...
    def generateWheelSpeeds(self, file_name: str, steps: int = 1, separate_lines: bool = False):
        with open('{0}.txt'.format(file_name), "w") as f:
            ...
            while t <= rg:
                ...
                wheel_states = self.sim.robot.kinematics.inverse(robot_centric_vel)

                if isinstance(self.sim.robot.kinematics, SwerveKinematics):
                    # add module angles too
                    line = ''.join("{0} {1} ".format(round(state.VELOCITY, 2), round(state.ANGLE, 2)) for state in wheel_states)
                else:
        ...
        with open('{0}.txt'.format(file_name), "w") as f:
            ...
            while t <= rg:
                ...
                line = '{0} {1} {2} {3} {4} '.format(
                    round(robot_centric_vel.VEL.x),
                    round(robot_centric_vel.VEL.y),
                    round(robot_centric_vel.ANG_VEL),
                    round(current_state.pose.head, 2),
                    consecutive)
